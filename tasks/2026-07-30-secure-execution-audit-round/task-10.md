# Task 10: EXEC-005 Process Tree Controller

## Objective

Implement EXEC-005 for Dokion at baseline `a1e1d55`. Track process group identity and terminate descendants using SIGTERM then bounded SIGKILL on supported POSIX hosts. Record reason, timing, exit status, and degraded guarantees.
...
Implement and export: `terminateProcessTree(handle, reason, options): Promise<ProcessTerminationResult>`

## Allowed production files

- `src/execution/process-controller.ts`

## Allowed test files

- `tests/execution/process-controller.test.ts`

## TDD contract

1. Add the focused test first.
2. Run `bun test tests/execution/process-controller.test.ts` and record the expected failing assertion or missing symbol.
3. Implement the minimum production code.
4. Run `bun test tests/execution/process-controller.test.ts` and `bun run typecheck`.

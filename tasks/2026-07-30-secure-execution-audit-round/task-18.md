# Task 18: EVID-001 Completion Criterion Evaluator

    ## Objective

    Implement EVID-001 for Dokion at baseline `a1e1d55`. Store PASS, FAIL, BLOCKED, or NOT_APPLICABLE for every criterion with evaluator version, evidence references, and freshness. Missing required criteria prevent completion.
    ...
    Implement and export: `evaluateCompletion(input): CompletionEvaluation`

    ## Allowed production files

    - `src/readiness/completion.ts`
- `schemas/dokion-state.schema.json`

    ## Allowed test files

    - `tests/readiness/completion.test.ts`

    ## TDD contract

    1. Add the focused test first.
    2. Run `bun test tests/readiness/completion.test.ts` and record the expected failing assertion or missing symbol.
    3. Implement the minimum production code.
    4. Run `bun test tests/readiness/completion.test.ts` and `bun run typecheck`.

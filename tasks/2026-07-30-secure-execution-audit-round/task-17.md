# Task 17: STATE-008 Safe Signal Interruption

    ## Objective

    Implement STATE-008 for Dokion at baseline `a1e1d55`. On SIGINT or SIGTERM, cancel child work, checkpoint interruption, append a typed event, mark or release the run lock, and leave a resumable non-failed state without replaying completed actions.
    ...
    Implement and export: `installSignalHandler(context): SignalHandlerController`

    ## Allowed production files

    - `src/runtime/signal-handler.ts`
- `src/engine/runtime-engine.ts`

    ## Allowed test files

    - `tests/state/signal-recovery.test.ts`

    ## TDD contract

    1. Add the focused test first.
    2. Run `bun test tests/state/signal-recovery.test.ts` and record the expected failing assertion or missing symbol.
    3. Implement the minimum production code.
    4. Run `bun test tests/state/signal-recovery.test.ts` and `bun run typecheck`.

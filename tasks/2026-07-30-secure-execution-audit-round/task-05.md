# Task 05: STATE-009 Side-Effect Checkpoint Store

## Objective

Implement STATE-009 for Dokion at baseline `a1e1d55`. Persist intent before external action and completion afterward with stable IDs, monotonic state transitions, atomic writes, and STARTED_UNKNOWN recovery semantics.
...
Implement and export: `beginSideEffect(root, intent): Promise<SideEffectCheckpoint>; completeSideEffect(root, id, outcome): Promise<SideEffectCheckpoint>`
...
2. Run `bun test tests/state/checkpoint.test.ts` and record the expected failing assertion or missing symbol.
3. Implement the minimum production code.
4. Run `bun test tests/state/checkpoint.test.ts` and `bun run typecheck`.

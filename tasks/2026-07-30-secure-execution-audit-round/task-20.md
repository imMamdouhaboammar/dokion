# Task 20: EVID-003 Declared Execution Report Section

## Objective

Implement EVID-003 for Dokion at baseline `a1e1d55`. List every stage and step in declared order with actual disposition, capability identity, version, digest, attempts, and evidence. Keep skipped, blocked, failed, and unexecuted work visible.
...
Implement and export: `renderExecutionSection(input): ExecutionSection`

## Allowed production files

- `src/report/sections/execution.ts`

## Allowed test files

- `tests/report/execution-section.test.ts`

## TDD contract

1. Add the focused test first.
2. Run `bun test tests/report/execution-section.test.ts` and record the expected failing assertion or missing symbol.
3. Implement the minimum production code.
4. Run `bun test tests/report/execution-section.test.ts` and `bun run typecheck`.

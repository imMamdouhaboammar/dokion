# Task 04: CAP-008 Deterministic Doctor Integration

    ## Objective

    Implement CAP-008 for Dokion at baseline `a1e1d55`. Integrate provenance, prerequisite, digest, availability, conflict, and degradation checks into a read-only deterministic doctor report with blocking and warning severity.
    ...
    Implement and export: `runDoctorAudit(root: string): Promise<DoctorAuditReport>`
...
- `src/cli/handlers/doctor.ts`
    ...
    2. Run `bun test tests/cli/doctor.test.ts` and record the expected failing assertion or missing symbol.
    3. Implement the minimum production code.
    4. Run `bun test tests/cli/doctor.test.ts` and `bun run typecheck`.

# Task 14: EXEC-008 Binary and Large File Snapshot

## Objective

Implement EXEC-008 for Dokion at baseline `a1e1d55`. Detect text versus binary without lossy conversion, record metadata and digest, copy exact bytes only within bounds, and fail before mutation when exact rollback cannot be proven.
...
Implement and export: `captureFileSnapshot(path, options): Promise<FileSnapshot>`

## Allowed production files

- `src/validation/file-snapshot.ts`

## Allowed test files

- `tests/validation/binary-snapshot.test.ts`

## TDD contract

1. Add the focused test first.
2. Run `bun test tests/validation/binary-snapshot.test.ts` and record the expected failing assertion or missing symbol.
3. Implement the minimum production code.
4. Run `bun test tests/validation/binary-snapshot.test.ts` and `bun run typecheck`.

# Task 03: CAP-007 Capability Conflict Detection

## Objective

Implement CAP-007 for Dokion at baseline `a1e1d55`. Detect incompatible versions, duplicate responsibility, overlapping write scopes in unsafe parallel stages, and platform incompatibility. Never auto-resolve by substitution or reordering.
...
Implement and export: `detectCapabilityConflicts(capabilities, platform): CapabilityConflict[]`

## Allowed production files

- `src/capability/conflict-detector.ts`

## Allowed test files

- `tests/capability/conflict-detector.test.ts`

## TDD contract

1. Add the focused test first.
2. Run `bun test tests/capability/conflict-detector.test.ts` and record the expected failing assertion or missing symbol.
3. Implement the minimum production code.
4. Run `bun test tests/capability/conflict-detector.test.ts` and `bun run typecheck`.

# Task 07: EXEC-002 Argument-Vector Command Specification

    ## Objective

    Implement EXEC-002 for Dokion at baseline `a1e1d55`. Support executable plus argument array and retain legacy shell strings as explicit high-risk specs. Reject mixed or empty forms and preserve executable and arguments separately.
    ...
    Implement and export: `normalizeCommandSpec(input): NormalizedCommandSpec`

    ## Allowed production files

    - `src/execution/command-spec.ts`
- `schemas/dokion-playbook.schema.json`

    ## Allowed test files

    - `tests/execution/command-spec.test.ts`

    ## TDD contract

    1. Add the focused test first.
    2. Run `bun test tests/execution/command-spec.test.ts` and record the expected failing assertion or missing symbol.
    3. Implement the minimum production code.
    4. Run `bun test tests/execution/command-spec.test.ts` and `bun run typecheck`.

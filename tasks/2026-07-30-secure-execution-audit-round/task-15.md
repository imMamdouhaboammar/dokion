# Task 15: EXEC-009 Repair Transaction Manifest

## Objective

Implement EXEC-009 for Dokion at baseline `a1e1d55`. Represent before snapshot, command, after snapshot, diff, validation, verification, rollback, digests, changed paths, checkpoints, and final disposition in one versioned transaction.
...
Implement and export: `createRepairTransaction(input): RepairTransaction; advanceRepairTransaction(transaction, event): RepairTransaction`

## Allowed production files

- `src/validation/repair-transaction.ts`

## Allowed test files

- `tests/validation/repair-transaction.test.ts`

## TDD contract

1. Add the focused test first.
2. Run `bun test tests/validation/repair-transaction.test.ts` and record the expected failing assertion or missing symbol.
3. Implement the minimum production code.
4. Run `bun test tests/validation/repair-transaction.test.ts` and `bun run typecheck`.

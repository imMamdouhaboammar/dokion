# Task 21: EVID-004 Exceptions Report Section

## Objective

Implement EVID-004 for Dokion at baseline `a1e1d55`. Render skips, manual reviews, accepted risks, deferrals, blocked lanes, stale evidence, degradations, and inert recommendations with actor and reason.
...
Implement and export: `renderExceptionsSection(input): ExceptionsSection`

## Allowed production files

- `src/report/sections/exceptions.ts`

## Allowed test files

- `tests/report/exceptions-section.test.ts`

## TDD contract

1. Add the focused test first.
2. Run `bun test tests/report/exceptions-section.test.ts` and record the expected failing assertion or missing symbol.
3. Implement the minimum production code.
4. Run `bun test tests/report/exceptions-section.test.ts` and `bun run typecheck`.

# Task 08: EXEC-003 Child Environment Policy

## Objective

Implement EXEC-003 for Dokion at baseline `a1e1d55`. Construct an allowlisted child environment from platform-safe defaults and declared variables. Deny loader injection variables and expose redaction tokens without secret values.
...
Implement and export: `buildChildEnvironment(input): ChildEnvironmentResult`

## Allowed production files

- `src/execution/environment-policy.ts`

## Allowed test files

- `tests/execution/environment-policy.test.ts`

## TDD contract

1. Add the focused test first.
2. Run `bun test tests/execution/environment-policy.test.ts` and record the expected failing assertion or missing symbol.
3. Implement the minimum production code.
4. Run `bun test tests/execution/environment-policy.test.ts` and `bun run typecheck`.

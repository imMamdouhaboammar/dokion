# Task 11: Secure Command Runner Integration

## Objective

Implement EXEC-001..005 for Dokion at baseline `a1e1d55`. Integrate command strategy, normalized specs, child environment policy, bounded output spooling, timeout handling, and process-tree termination while preserving legacy callers.
...
Implement and export: `runCommand(root, commandSpec, options): Promise<CommandResult>`
...
- `tests/execution/secure-command-runner.test.ts`

## TDD contract

1. Add the focused test first.
2. Run `bun test tests/execution/secure-command-runner.test.ts` and record the expected failing assertion or missing symbol.
3. Implement the minimum production code.
4. Run `bun test tests/execution/secure-command-runner.test.ts` and `bun run typecheck`.

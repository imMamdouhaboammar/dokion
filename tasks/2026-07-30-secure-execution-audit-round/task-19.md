# Task 19: EVID-002 Qualified Readiness Statement

## Objective

Implement EVID-002 for Dokion at baseline `a1e1d55`. Tie wording to subject, commit, playbook digest, lock digest, gates, coverage, degradations, exclusions, and timestamp. Reject unqualified production-ready language.
...
Implement and export: `formatReadinessStatement(input): QualifiedReadinessStatement`

## Allowed production files

- `src/readiness/readiness-statement.ts`

## Allowed test files

- `tests/readiness/statement.test.ts`

## TDD contract

1. Add the focused test first.
2. Run `bun test tests/readiness/statement.test.ts` and record the expected failing assertion or missing symbol.
3. Implement the minimum production code.
4. Run `bun test tests/readiness/statement.test.ts` and `bun run typecheck`.

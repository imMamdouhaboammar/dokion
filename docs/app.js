/* Dokion Community Playbook Hub — Client Application Logic */
...
function renderGrid(data, container) {
  if (data.length === 0) {
    container.innerHTML = `<div style="grid-column: 1/-1; text-align: center; padding: 40px; color: var(--text-muted);">No playbooks found matching criteria.</div>`;
    return;
  }
        ...
        <div class="playbook-card">
          ...
          <div>
            ...
            <div class="card-footer">
              <button class="btn btn-primary" onclick="copyPullCommand('${p.id}')">Copy Pull Cmd</button>
    ...
    .map((p, idx) => {
      ...
      return `
        <tr>
          ...
          <td>
            <button class="btn btn-secondary" style="padding: 6px 12px; font-size: 12px;" onclick="openInspectModal('${p.id}')">View</button>
          </td>
        </tr>
      `;
    })
...
function openInspectModal(packageId) {
  ...
  modalBody.innerHTML = `
    ...
    <div style="display: flex; gap: 16px; font-size: 13px; color: var(--text-dim); margin-bottom: 20px;">
      <span>Publisher: <strong>${p.publisher.name}</strong></span>
      <span>Category: <strong>${p.category}</strong></span>
      <span>Digest: <code>${p.digest.substring(0, 18)}...</code></span>
    </div>

    <div class="code-block">
      <span>dokion hub pull ${p.id}</span>
      <button class="btn btn-secondary" style="padding: 4px 10px; font-size: 12px;" onclick="copyPullCommand('${p.id}')">Copy</button>
    </div>

    <h4 style="font-size: 15px; font-weight: 700; margin-top: 20px; margin-bottom: 10px;">Declared Stages:</h4>

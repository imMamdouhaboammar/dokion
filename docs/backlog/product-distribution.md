...
Turn the verified engine into an installable and understandable product. Every adapter must preserve the same authority model and every public claim must be bound to tested release evidence.
...
- Acceptance: Adapter metadata cannot add commands, defaults, capability selection, or execution authority unavailable to the core CLI.
...
- Acceptance: Extension validation passes and command text remains generated or contract-checked against the core registry.
...
- Acceptance: A weaker platform cannot silently satisfy a blocking requirement and every degradation appears in plan, status, report, and sign-off.
...
- Acceptance: Cross-compilation alone never marks a host tested and failures block the corresponding compatibility claim.
...
- Acceptance: Handoff cannot weaken authority or duplicate side effects. Unsupported guarantee changes require review.
...
- Deliverable: Align help, parsing, JSON output, diagnostics, exit codes, registry, examples, and status summaries for the complete public-beta surface.
- Acceptance: Unknown options and invalid states return stable codes. Stdout contains results, stderr contains diagnostics, and adapters remain in parity.
...
- Acceptance: Commands are tested from clean fixtures and a new user can complete the beta journey without private repository knowledge.
...
- Deliverable: Reproducible web, API, and library fixtures with known defects, fake repairs, approvals, interruptions, and expected evidence.
- Acceptance: Fixtures reset deterministically, avoid unnecessary network-only dependencies, and remain versioned with expected outcomes.
...
- Acceptance: A safety-critical regression cannot merge through an unrelated green aggregate job and failure evidence remains available.
...
- Deliverable: Generate package and binary inventories, checksums, SBOM, synchronized versions, provenance attestations where supported, and partial-release rollback checks.

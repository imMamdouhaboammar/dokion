...
This workstream makes every result independently checkable. Human and machine reports are derived from validated state and evidence, never from model confidence or unsupported claims.
...
- Acceptance: A run cannot become complete while a required criterion is absent, stale, or unreconciled.
...
- Deliverable: Generate one statement tied to subject, repository commit, playbook digest, lock digest, gates, coverage, degradations, exclusions, and time.
...
- Acceptance: Unexecuted, skipped, blocked, and failed work remains visible and order matches the active playbook.
...
- Acceptance: Export validates against fixture expectations and does not convert accepted or deferred work into fixed results.
...
- Deliverable: Export stages, steps, verification commands, and gates as deterministic test suites for CI consumption.
- Acceptance: Blocked, skipped, failed, and errored outcomes remain distinct and export never changes Dokion state.
...
- Acceptance: `dokion audit` can run without executing capabilities and returns precise failures for every tampered artifact fixture.

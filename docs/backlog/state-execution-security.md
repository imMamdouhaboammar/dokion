...
Prevent concurrent or stale execution and contain every external side effect. Recovery must be deterministic and repairs must be reversible before mutation begins.

P0 blocks public-beta promotion. P1 blocks a broader production-grade claim unless the surface is explicitly excluded.

## State and recovery backlog
...
- Deliverable: A lock containing process identity, run identity, timestamp, host, and recovery metadata for run, resume, step, reset, verify, and autopilot.
...
- Deliverable: Increment state revision on every accepted transition and require the expected revision for updates.
...
- Deliverable: Add previous and current digests plus a verifier for deletion, insertion, modification, truncation, and cross-run splicing.
- Acceptance: Journal verification fails closed and identifies the first invalid sequence without rewriting evidence.
...
- Acceptance: Pre-existing changes are distinguished from Dokion changes and write-capable work blocks when rollback cannot be preserved.
...
- Deliverable: On SIGINT and SIGTERM, stop child work, persist a checkpoint, append an interruption event, update the lock, and leave the run resumable.
...
- Acceptance: Recovery distinguishes not-started, started-unknown, and completed outcomes for every side effect.
...
- Deliverable: Explicit POSIX shell strategies for claimed Linux and macOS surfaces and an explicit compatible strategy for Windows.
...
- Deliverable: Allow executable plus argument-array declarations while retaining legacy shell strings as explicit higher-risk commands.
- Acceptance: Argument vectors bypass shell parsing and command identity and arguments remain separately visible in evidence.
...
- Acceptance: Dangerous loader variables are denied, undeclared values are absent, and redaction covers output, events, evidence, reports, and errors.
...
- Acceptance: Large output cannot exhaust memory. Truncation is explicit and evidence limits participate in run budgets.
...
- Acceptance: Policy decisions use canonical repository-relative paths and record attempted and declared scopes.
...
- Acceptance: Declared ignored files participate in before, after, and rollback evidence. Cap exhaustion blocks before mutation.
...
- Deliverable: Seeded and generated cases for globs, Unicode, symlinks, case behavior, ignored files, binaries, suppression, test weakening, large diffs, and rollback.

...
Prove exactly which executable components were used and introduce typed assurance modules. A catalog entry is descriptive only. Execution requires an active playbook declaration and a verified capability lock entry.

P0 blocks public-beta promotion. P1 blocks a broader production-grade claim unless the surface is explicitly excluded.

## Capability provenance backlog
...
- Deliverable: Persist identity, version, source, executable path, digest, provenance, installer exception, platform, and verification time.
- Acceptance: Lock writes are atomic and schema-valid. Stale, altered, future-version, or authority-claiming locks fail closed.
...
- Deliverable: Resolve only the executable declared by the active playbook through an allowed PATH and canonical path rules.
- Acceptance: Ambiguous matches, missing executables, symlink escapes, and undeclared fallbacks block before execution.
...
- Deliverable: Support exact, range, and command-output version rules without interpreting output as instructions.
- Acceptance: Mismatch, malformed output, timeout, and conflicting declarations produce stable blocking results.
...
- Deliverable: Canonical hashing for files and directories plus verification of repository URL and commit SHA.
- Acceptance: Mutable branch-only references, changed files, path-order variation, and source mismatch are rejected with evidence.
...
- Deliverable: Record package manager, registry, package, version, integrity, installer command, exception reason, approval, and verifier.
- Acceptance: Non-Bun installation is accepted only when declared and approved. Credentials never enter state, evidence, reports, or logs.
...
- Deliverable: Check presence and allowed shape of declared variables and pass only declared values to commands.
...
- Acceptance: Conflicts become deterministic blocking results and cannot be resolved through substitution or reordering.
...
- Acceptance: `dokion doctor` is read-only, deterministic in JSON mode, redacts secrets, and distinguishes blockers from warnings.

## Assurance module backlog
...
- Deliverable: A schema for identity, version, responsibility, inputs, outputs, commands, permissions, platforms, findings mapping, and verification contract.
...
- Deliverable: Typed prepare, analyze, remediate, verify, and summarize inputs and outputs with explicit context.
...
- Deliverable: Load a module only when referenced by the active playbook and verified lock.
...
- Acceptance: Any requested expansion blocks before command execution and records the denied permission.
...
- Acceptance: Malformed output, missing artifacts, out-of-scope writes, timeout, and undeclared commands fail without fabricated findings.
...
- Deliverable: Add approved static-analysis, secret-review, authorization-evidence, injection-risk, and security-configuration adapters.
- Acceptance: Rule provenance remains visible, automated and manual coverage remain distinct, and no module executes unless selected by the active playbook.
...
- Deliverable: Add logging, redaction, health, timeout, retry, idempotency, benchmark, web-vitals, and accessibility artifact adapters.
...
- Deliverable: Prevent AI safety and mobile-native security from appearing covered without assigned modules and evidence.

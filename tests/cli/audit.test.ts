...
describe('EVID-010 Independent Audit Command', () => {
  test('computes evidence manifest and runs audit cleanly', async () => {
    const result = await handleAuditCommand([
    ...
    expect(result.message).toContain('verified 2 evidence artifact');
  });
});

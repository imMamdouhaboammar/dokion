...
export async function assertSafeRegularFilePath(
  ...
  for (let index = 1; index < parts.length; index += 1) {
    ...
    if (!stat) unsafe(code, "File path does not exist.", path, { component: current });
    if (stat.isSymbolicLink()) {
      unsafe(code, "File path may not contain symbolic links.", path, { component: current });
    }
    const final = index === parts.length - 1;
    if (!final && !stat.isDirectory()) {
      unsafe(code, "File path components must be directories.", path, { component: current });
    }
    if (final && (!stat.isFile() || stat.nlink !== 1)) {
      unsafe(code, "File path must be a single-link regular file.", path, {
        component: current,
        links: stat.nlink
      });
    }
...
export async function ensureSafeDirectoryPath(
  ...
  for (let index = 1; index < parts.length; index += 1) {
    ...
    if (!stat || stat.isSymbolicLink() || !stat.isDirectory()) {
      unsafe(code, "Directory path may contain only real directories.", path, {
        component: current,
